...
function renderPRTable(prs) {
    ...
    const canApprove = user && user.role === 'Manager';
    ...
    tbody.innerHTML = prs.map(pr => {
        ...
        const actions = [];
        actions.push(`
            <button class="btn btn-outline-primary" onclick="viewPR(${pr.pr_id})">
        ...
        if (canApprove && status === 'pending') {
            actions.push(`
                <button class="btn btn-outline-success" onclick="approvePR(${pr.pr_id})">
                    <i class="fas fa-check"></i>
                </button>
                <button class="btn btn-outline-danger" onclick="rejectPR(${pr.pr_id})">
