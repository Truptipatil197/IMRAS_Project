...
async function loadStaffStats() {
    ...
    try {
        // Fetch stats
        const [grnsRes, ledgerRes] = await Promise.all([
            fetchWithAuth(`${CONFIG.API_BASE_URL}/api/grn/pending-pos`).catch(() => null),
            fetchWithAuth(`${CONFIG.API_BASE_URL}/api/stock/ledger?limit=50`).catch(() => null)
        ]);
        ...
        if (grnsRes && grnsRes.ok) {
            const json = await grnsRes.json();
            // Backend wraps: { success, message, data }
            const payload = json && json.data ? json.data : json || {};
            const grns = Array.isArray(payload) ? payload : (Array.isArray(payload.grns) ? payload.grns : []);
            pendingGRNs = Array.isArray(grns) ? grns.length : 0;
        }
        ...
        if (ledgerRes && ledgerRes.ok) {
            ...
            todayMovements = Array.isArray(movements) 
                ? movements.filter(m => (m.transaction_date && m.transaction_date.startsWith(today)) || (m.created_at && m.created_at.startsWith(today))).length 
                : 0;
        }
                ...
                <table class="table table-sm">
                    ...
                    <tbody>
                        ${movements.map(movement => {
                            ...
                            return `
                                <tr>
                                    <td><small>${formatRelativeTime(movement.transaction_date || movement.createdAt)}</small></td>
                                    <td>${itemName}</td>
...
function processGRN(poId) {
    ...
    // Navigate to GRN processing page – actual processing handled in grn.html script
    if (poId) {
        window.location.href = `grn.html?po_id=${poId}`;
    } else {
