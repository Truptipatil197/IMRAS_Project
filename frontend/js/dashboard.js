...
function filterSidebarByRole() {
    ...
    // Links mapping for special logic
    const adminOnlyPaths = ['user-management.html', 'settings.html'];
    const managerAllowedPaths = ['reorder-automation.html'];
    const staffOperationalPaths = ['grn.html'];

    const navLinks = sidebar.querySelectorAll('.nav-link');
    navLinks.forEach(link => {
        const href = link.getAttribute('href') || '';
        let hideForThisRole = false;

        // Restriction 1: Admin-only modules for everyone else
        if (adminOnlyPaths.some(p => href.includes(p)) && role !== 'admin') {
            hideForThisRole = true;
        }

        // Restriction 2: Automation access for Manager and Admin
        if (managerAllowedPaths.some(p => href.includes(p)) && role !== 'admin' && role !== 'manager') {
            hideForThisRole = true;
        }

        // Restriction 3: Goods Receipt is for Staff only, hide for Admin/Manager
        if (staffOperationalPaths.some(p => href.includes(p)) && role !== 'staff') {
            hideForThisRole = true;
        }
