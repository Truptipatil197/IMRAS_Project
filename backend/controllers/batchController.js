...
const getExpiryAlerts = async (req, res) => {
  try {
    ...
    // Count queries with same where clause
    const countWhere = { ...where };
    const unread_count = await Alert.count({ 
      where: { ...countWhere, is_read: false } 
    });
    const critical_count = await Alert.count({ 
      where: { ...countWhere, severity: 'Critical' } 
    });

    const summary = {
      expired_items: await Alert.count({ 
        where: { ...countWhere, alert_type: 'Expired' } 
      }),
      expiring_7days: await Alert.count({ 
        where: { ...countWhere, alert_type: 'Expiry Warning - 7 Days' } 
      }),
      expiring_30days: await Alert.count({ 
        where: { ...countWhere, alert_type: 'Expiry Warning - 30 Days' } 
      })
    };
