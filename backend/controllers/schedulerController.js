 ...
 * FIXED: Wait for execution to complete and return proper result
 */
exports.runSchedulerNow = async (req, res) => {
  try {
    ...
    console.log('[REORDER] Run Now triggered by user', req.user.user_id);

    // FIXED: Wait for execution to complete and return result
    const result = await reorderScheduler.runNow(req.user.user_id);

    // Also trigger alert generation manually so the user sees results immediately
    // We don't wait for this one to keep response fast, or we can wait if we want to guarantee alerts
    // Let's run it in background but log it
    alertScheduler.runNow().catch(err => console.error('Manual alert generation failed:', err));
    ...
    if (result.success) {
      return res.status(200).json({
        ...
        items_checked: result.stats.itemsProcessed,
        alerts_generated: result.stats.alertsCreated,
        prs_created: result.stats.prsGenerated,
        duration_seconds: Math.round(result.stats.executionTime / 1000),
