...
const getABCAnalysis = async (req, res) => {
  try {
    ...
    // Latest stock per item (sum across warehouses or filtered)
    ...
    for (const l of ledgers) {
      if (category_id && l.item?.category_id !== parseInt(category_id, 10)) continue;
      const stockQty = Number(l.balance_qty || 0);
      const unitPrice = Number(l.item?.unit_price || 0);
      ...
      items.push({
        ...
        category_name: l.item?.category?.category_name || null,
...
const getStockTurnoverAnalysis = async (req, res) => {
  try {
    ...
    const start_date_str = startDate.toISOString().split('T')[0];
    const end_date_str = endDate.toISOString().split('T')[0];
    const days = Math.max(1, Math.floor((new Date(end_date) - new Date(start_date)) / (1000 * 60 * 60 * 24)));
    ...
    for (const it of items) {
      ...
      itemResults.push({
        ...
        efficiency_rating: turnover > 3 ? 'A+' : turnover > 2 ? 'A' : turnover > 1 ? 'B' : 'C'
      });
    }
    ...
    const response = {
      ...
      overall_metrics: {
        ...
        avg_days_of_supply: overallTurnover > 0 ? Number((365 / overallTurnover).toFixed(1)) : null
      },
      items: itemResults
    };
...
const getWarehousePerformance = async (req, res) => {
  try {
    ...
    for (const wh of warehouses) {
      // Latest balances per item in this warehouse
      const latest = await StockLedger.findAll({
      ...
      const ledgerIds = latest.map(l => l.latest_ledger_id);
      const ledgers = ledgerIds.length ? await StockLedger.findAll({ where: { ledger_id: { [Op.in]: ledgerIds } } }) : [];
      const currentStockUnits = ledgers.reduce((sum, l) => sum + Number(l.balance_qty || 0), 0);
      const totalStockValue = ledgers.reduce((sum, l) => sum + calculateInventoryValue(l.balance_qty, l.item?.unit_price || 0), 0);
      ...
      results.push({
        ...
        metrics: {
          ...
          number_of_items: latest.length,
          number_of_active_batches: null,
          turnover_ratio: null,
          avg_issue_fulfillment_time_hours: null,
          inventory_accuracy: null,
          adjustments_last_30days: null,
          expiring_items_count: null,
          performance_rating: 'N/A'
        },
        recommendations: [
          'Review warehouse KPIs once capacity and accuracy metrics are available'
        ]
      });
    }
...
const getExecutiveDashboard = async (req, res) => {
  try {
    // Basic snapshot using existing helpers
    const abc = await StockLedger.count();
    return ok(res, {
      generated_at: new Date().toISOString(),
      kpis: {
        total_inventory_value: null,
        monthly_turnover_ratio: null,
        inventory_accuracy: null,
        on_time_delivery_rate: null,
        stockout_incidents_month: null,
        fill_rate: null
      },
      alerts_summary: null,
