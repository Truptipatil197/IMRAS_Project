...
const getItemSuppliers = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const data = suppliers.map(s => ({
      ...
      supplier_name: s.supplier?.supplier_name || 'Unknown',
      unit_price: s.unit_price,
      is_preferred: s.is_preferred,
      min_order_qty: s.min_order_qty
    }));

    return ok(res, data, 'Item suppliers retrieved');
  } catch (error) {
    console.error('getItemSuppliers error:', error);
    return fail(res, 'Failed to fetch item suppliers', 500);
  }
};
