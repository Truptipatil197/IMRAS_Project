...
// DEPRECATED: Standardized logic in stockUtils.js
const getCurrentStockMaps = async (transaction = null) => {
  const stockMap = await getAllItemsStock(null, transaction);

  // Calculate stock per item per warehouse
  const warehouseStockResults = await StockLedger.findAll({
    attributes: [
      'item_id',
      'warehouse_id',
  ...
  const perWarehouse = {};
  warehouseStockResults.forEach(result => {
    const itemId = result.item_id;
    const warehouseId = result.warehouse_id;
    const qty = parseFloat(result.current_stock || 0);

    if (!perWarehouse[itemId]) {
      perWarehouse[itemId] = [];
    }

    perWarehouse[itemId].push({
      warehouse_id: warehouseId,
      warehouse_name: result.warehouse ? result.warehouse.warehouse_name : null,
      current_stock: qty
    });
  });
...
const getCurrentStockForItem = async (item_id, transaction = null) => {
  return getCurrentStock({ item_id }, transaction);
};
...
// FIXED: Low stock alerts now use correct stock calculation
const checkReorderPoints = async (req, res) => {
  try {
    const items = await Item.findAll({
      where: { is_active: true },
      attributes: ['item_id', 'sku', 'item_name', 'reorder_point', 'min_stock', 'max_stock', 'safety_stock', 'unit_price']
    });

    // FIXED: Use correct stock calculation from stock_ledgers
    ...
    for (const item of items) {
      ...
      // FIXED: Use min_stock (or reorder_point as fallback)
      ...
      if (currentStock <= minStock) {
        ...
        const recommended_order_qty = Math.max(maxStock - currentStock, minStock);
        ...
        if (!existingAlert) {
          await Alert.create({
            ...
            message: `Item ${item.item_name} stock is ${currentStock}, below min stock of ${minStock}`,
...
const getReorderAlerts = async (req, res) => {
  console.log('[DEBUG] getReorderAlerts called');
  try {
    ...
    const alerts = await Alert.findAll({
      where: whereClause,
      include: [], // Removed includes to isolate 500 cause
      order: [
    ...
    for (const a of alerts) {
      ...
      const alertObj = {
        ...
        item: a.item ? {
          ...
          min_stock: a.item.min_stock,
          max_stock: a.item.max_stock,
      ...
      // Hybrid: Add legacy flattened fields for Automation Grid
      if (a.item) {
        // Manual fetch for stability
        let preferredSupplier = null;
        try {
          preferredSupplier = await SupplierItem.findOne({
            where: { item_id: a.item.item_id, is_preferred: true },
            include: [{ model: Supplier, attributes: ['supplier_id', 'supplier_name'] }]
          });
        } catch (err) {
          console.warn(`Failed to fetch supplier for item ${a.item.item_id}:`, err.message);
        }
        ...
        Object.assign(alertObj, {
          ...
          reorder_point: a.item.reorder_point,
          safety_stock: a.item.safety_stock,
          lead_time_days: a.item.lead_time_days || 7,
          preferred_supplier: preferredSupplier?.Supplier?.supplier_name || null,
          supplier_id: preferredSupplier?.supplier_id || null,
          supplier_unit_price: parseFloat(preferredSupplier?.unit_price || a.item.unit_price || 0),
          min_order_qty: parseFloat(preferredSupplier?.min_order_qty || 1)
        });
      }

      alertPayload.push(alertObj);
    }

    return ok(res, {
      success: true, // Legacy flag
      unread_count,
      critical_count,
      alerts: alertPayload
  ...
  try {
    ...
    const mapped = rows.map(pr => {
      ...
      return {
        ...
        po_id: pr.purchaseOrders && pr.purchaseOrders.length > 0 ? pr.purchaseOrders[0].po_id : null
      };
    });
  ...
  try {
    ...
    const pr = await PurchaseRequisition.findByPk(id, {
      include: [
        ...
        { model: User, as: 'approver', attributes: ['user_id', 'full_name'], required: false },
        { model: PRItem, as: 'prItems', include: [{ model: Item, as: 'item' }] },
        { model: PurchaseOrder, as: 'purchaseOrders', attributes: ['po_id', 'po_number', 'status'], required: false }
      ]
    });
...
const rejectPurchaseRequisition = async (req, res) => {
  try {
    ...
    const finalReason = rejection_reason || remarks;

    if (!finalReason || finalReason.length < 5) {
      return fail(res, 'rejection_reason or remarks is required (min 5 chars)', 400);
    }
    ...
    if (pr.requester?.email) {
      await sendPRRejectionNotification(pr.pr_id, pr.requester.email, finalReason);
    }
...
const createPurchaseOrderFromPR = async (req, res) => {
  ...
  try {
    ...
    const expectedDateStr = new Date(expected_delivery_date).toISOString().split('T')[0];
    const todayStr = new Date().toISOString().split('T')[0];

    if (!expected_delivery_date || expectedDateStr < todayStr) {
      await transaction.rollback();
      return fail(res, 'expected_delivery_date must be today or a future date', 400);
    }
    ...
    console.log(`[PO-CONV] Created PO for PR ${pr.pr_number}`);
    ...
    if (supplier.email) {
      try {
        ...
        console.error('[PO-CONV] Notification failed:', emailError.message);
      }
    }
...
// FIXED: Get items requiring reorder with pending PR check
const getItemsRequiringReorder = async (req, res) => {
  try {
    const items = await Item.findAll({
      where: { is_active: true },
      attributes: ['item_id', 'sku', 'item_name', 'reorder_point', 'min_stock', 'max_stock', 'safety_stock', 'unit_price'],
    ...
    for (const item of items) {
      ...
      // Only include items where stock <= min_stock
      if (minStock > 0 && currentStock <= minStock) {
        // Check if pending PR already exists
        const pendingPR = await PurchaseRequisition.findOne({
          include: [{
            model: PRItem,
            as: 'prItems',
        ...
        const status = currentStock === 0 ? 'Out of Stock' : currentStock < item.safety_stock ? 'Critical' : 'Low Stock';
        ...
        itemsNeedingReorder.push({
          ...
          estimated_cost: reorderQty * (item.unit_price || 0),
          status,
          has_pending_pr: !!pendingPR,
          pending_pr_number: pendingPR ? pendingPR.pr_number : null
        });
      }
    }

    // Sort by status (Out of Stock > Critical > Low Stock)
    ...
    return ok(res, {
      ...
      items: itemsNeedingReorder
    }, 'Items requiring reorder retrieved successfully');
  } catch (error) {
    console.error('getItemsRequiringReorder error:', error);
    return fail(res, 'Failed to fetch items requiring reorder', 500);
  }
};
...
const getAllPurchaseOrders = async (req, res) => {
  try {
    ...
    const whereClause = {};
    if (status) whereClause.status = status;
    if (supplier_id) whereClause.supplier_id = parseInt(supplier_id, 10);
    ...
    const { rows, count } = await PurchaseOrder.findAndCountAll({
      ...
      include: [
        ...
        { model: POItem, as: 'poItems', attributes: ['item_id', 'ordered_qty'] },
    ...
    return fail(res, 'Failed to fetch purchase orders', 500);
  }
};

const createBulkPurchaseRequisition = async (req, res) => {
  const transaction = await sequelize.transaction();
  try {
    const { item_ids = [], remarks } = req.body;
    if (!item_ids.length) {
      await transaction.rollback();
      return fail(res, 'item_ids array must contain at least one ID', 400);
    }
    ...
    const pr = await PurchaseRequisition.create({
      ...
      status: 'Pending',
      remarks: remarks || `Bulk request for ${item_ids.length} items`
    }, { transaction });
    ...
    for (const itemId of item_ids) {
      ...
      const requestedQty = Math.max((item.reorder_point || 0) + (item.safety_stock || 0) - currentStock, 1);

      await PRItem.create({
        pr_id: pr.pr_id,
        item_id: itemId,
        requested_qty: requestedQty,
        justification: 'Automated bulk replenishment request'
      }, { transaction });
      itemsCreated++;
    }

    await transaction.commit();
    return ok(res, { pr_id: pr.pr_id, items_count: itemsCreated }, 'Bulk Purchase Requisition created', 201);
  } catch (error) {
    await transaction.rollback();
    console.error('createBulkPurchaseRequisition error:', error);
    return fail(res, error.message || 'Failed to create bulk PR', 500);
  }
};
