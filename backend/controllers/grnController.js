...
/**
 * Generate GRN number: GRN + YEAR + 5-digit sequential number
 */
const generateGRNNumber = async () => {
  ...
  let sequence = 1;
  if (lastGRN) {
    const lastNumber = lastGRN.grn_number.match(/\d+$/);
    if (lastNumber) {
  ...
  try {
    ...
    for (const po of pos) {
      ...
      for (const poItem of po.poItems) {
        ...
        const receivedQty = await GRNItem.sum('accepted_qty', {
          include: [{
            model: GRN,
            as: 'grn',
      ...
      // Only include PO if it has pending items
      if (items.length > 0) {
        pendingPOs.push({
          po_id: po.po_id,
    ...
    return ok(res, pendingPOs, 'Pending POs retrieved successfully');
  } catch (error) {
    console.error('Get pending POs error:', error);
    return fail(res, 'Failed to fetch pending purchase orders', 500);
  }
};
