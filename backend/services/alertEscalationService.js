  ...
  async processEscalations() {
    try {
      ...
      for (const alert of alerts) {
        const createdDate = alert.created_at || alert.createdAt || new Date();
        const startTime = (createdDate instanceof Date) ? createdDate.getTime() : Date.parse(createdDate);
        const hoursSinceCreation = (Date.now() - (startTime || Date.now())) / (60 * 60 * 1000);
