...
class ReorderScheduler {
  ...
  async executeReorderCheck(triggeredBy = 'scheduler', userId = null) {
    ...
    try {
      ...
      console.log('[REORDER] Scheduler triggered', new Date());

      // FIXED: Step 1: Check all items and automatically create PRs (no queue needed)
      logger.info('Step 1: Checking inventory levels and creating PRs...');
      const checkStats = await reorderService.checkAllItems(log.log_id || log.getDataValue('log_id'));

      // Step 2: Process reorder queue and generate PRs (for any remaining queue entries)
      logger.info('Step 2: Processing any remaining queue entries...');
      ...
      // FIXED: Update log with results (PRs are now created directly in checkAllItems)
      await log.markComplete({
        itemsProcessed: checkStats.itemsProcessed,
        itemsEligible: checkStats.itemsEligible,
        prsGenerated: (checkStats.prsCreated || 0) + prStats.successful,
        alertsCreated: (checkStats.alertsCreated || 0) + prStats.successful,
      ...
      logger.info(`Items processed: ${checkStats.itemsProcessed}, Eligible: ${checkStats.itemsEligible}, PRs generated: ${checkStats.prsCreated || 0} (direct) + ${prStats.successful} (queue)`);
